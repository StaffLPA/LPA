...
type Section = 'Overview' | 'Roster' | 'Calendar' | 'Group Chats' | 'Schedule Images' | 'Family Links' | 'Announcements';
...
function AdminDashboardContent() {
  ...
  return <View style={[styles.container, { backgroundColor: colors.background }]}>
    <ScrollView contentContainerStyle={[styles.content, { paddingTop: (Platform.OS === 'web' ? Math.max(insets.top, 67) : insets.top) + 18, paddingBottom: Math.max(insets.bottom, 34) + 36 }]} showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={[styles.nav, { backgroundColor: colors.muted }]} contentContainerStyle={styles.navContent}>{(['Overview', 'Announcements', 'Roster', 'Calendar', 'Group Chats', 'Schedule Images', 'Family Links'] as Section[]).map((item) => <Pressable key={item} testID={`admin-section-${item.toLowerCase().replaceAll(' ', '-')}`} onPress={() => setSection(item)} style={[styles.navButton, section === item && { backgroundColor: colors.card }]}><Text style={[styles.navText, { color: section === item ? colors.foreground : colors.mutedForeground }]}>{item}</Text></Pressable>)}</ScrollView>
