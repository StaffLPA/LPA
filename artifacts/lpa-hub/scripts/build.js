...
let metroPort = Number(process.env.EXPO_BUILD_METRO_PORT || 8082);
let metroBaseUrl = `http://localhost:${metroPort}`;
...
async function selectMetroPort() {
  for (let port = metroPort; port < metroPort + 20; port += 1) {
    if (await isPortAvailable(port)) {
      metroPort = port;
      metroBaseUrl = `http://localhost:${metroPort}`;
      if (port !== Number(process.env.EXPO_BUILD_METRO_PORT || 8082)) {
        console.log(`Metro build port was busy; using ${metroPort}`);
      }
      return;
    }
  }
  throw new Error(`No available Metro build port found near ${metroPort}.`);
}
...
function exportWeb(domain) {
  const outputDir = path.join(projectRoot, 'static-build', 'web');
  console.log('Exporting web application...');
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`Web export failed with exit code ${code}`));
    });
  });
}
